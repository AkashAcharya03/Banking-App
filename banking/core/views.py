...
from .forms import TransferForm, BillPaymentForm
...
def account_view(request):
    ...
    try:
        account = Account.objects.get(user=user)  # Retrieves the account for the logged-in user
...
def fund_transfer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            receiver_account = Account.objects.get(account_number=receiver_account.account_number)
...
def bill_payment(request):
    try:
        ...
        messages.error(request, "Multiple accounts found for the user. Please contact support.")
        return redirect('account')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            bill_payment.account = account  # Set the account to the logged-in user's account
            ...
            if account.balance >= bill_payment.amount:
                ...
                messages.success(request, f"Bill payment for {bill_payment.bill_name} of ₹{bill_payment.amount} was successful.")
                return redirect('home')
            else:
...
def home_view(request):
    ...
    return render(request, 'home.html', {'username': username, 'user': user, 'account': account})
...
def bill_payment_history(request):
    payments = BillPayment.objects.filter(account__user=request.user).order_by('-payment_date')
